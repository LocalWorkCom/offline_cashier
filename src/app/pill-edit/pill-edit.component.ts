...
export class PillEditComponent {
  ...
  test?: boolean;
  paymentMethod: any;
  cash_amountt: number | null = null;
  credit_amountt: number | null = null;
...
order_id:any ;
  ...
  saveOrder() {
    ...
    var cashAmount = this.cash_amountt != null ? this.cash_amountt : 0;
    var creditAmount = this.credit_amountt != null ? this.credit_amountt : 0;
 ...
 */    } catch (error) {
  ...
  isPaymentAmountValid(): boolean {
    ...
    const cash = Number(this.cash_amountt ?? 0);
    const credit = Number(this.credit_amountt ?? 0);
  ...
  show_delivered_only(aa: any) {
    ...
    this.cash_amountt = value;
    localStorage.setItem('cash_amountt', String(value));
  }

  setCreditAmount(value: number) {
    this.credit_amountt = value;
    localStorage.setItem('credit_amountt', String(value));
  }
  // setCashAmount(value: number | null): void {
  //   this.cash_amountt = Number((value ?? 0).toFixed(2));
  ...
  //   this.credit_amountt = Number((value ?? 0).toFixed(2));
  //   localStorage.setItem('credit_amountt', JSON.stringify(this.credit_amountt));
