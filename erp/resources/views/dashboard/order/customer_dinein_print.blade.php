...
<head>
    <title>{{ trans('order.Customer Receipt') }} ({{ trans('order.Dine In') }})</title>
    <style>
        body {
...
<body>
    ...
    <div class="section">
        ...
        <p>{{ trans('order.Table Number') }}: {{ $order->table_number ?? 'N/A' }}</p>
    </div>
    <div class="section">
        ...
        {{ trans('order.Discounts') }}: {{ $order->branch->country->currency_symbol }}{{ $order->discount }}</p>
