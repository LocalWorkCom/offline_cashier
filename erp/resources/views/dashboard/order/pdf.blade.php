...
<body>
    <div class="invoice-container">
        ...
        <div class="invoice-header">
            ...
            <div class="title">
              {{ trans('order.Order Number') }} <span class="order-number">{{ $order->order_number }}</span>
            </div>
        </div>
        ...
        <div class="billing-info">
            <div>
                <p class="fw-bold">{{ trans('order.Billing To') }}:</p>
        ...
        <div class="invoice-details">
            <p><strong>{{ trans('order.Invoice ID') }}:</strong> {{ $order->invoice_number }}</p>
            <p><strong>{{ trans('order.date') }}</strong>:</strong> {{ \Carbon\Carbon::parse($order->date)->format('d, M Y') }} -
                <span class="text-muted">{{ \Carbon\Carbon::parse($order->time)->format('h:i A') }}</span>
            </p>
            <p><strong>{{ trans('order.Due Amount') }}:</strong> ${{ $order->total_price_after_tax }}</p>
        </div>
            ...
            <tbody>
                ...
                @foreach ($order['addons'] as $addon)
                    <tr>
                        ...
                        <td> {{ $order->branch->country->currency_symbol }} {{ $addon->price_befor_tax }}</td>
                        <td> {{ $order->branch->country->currency_symbol }} {{ $addon->total }}</td>
                    </tr>
                @endforeach
                <!-- Totals Section -->
                <tr class="total-row">
                    <td colspan="3" class="text-end">{{ trans('order.Total Before Tax') }}:</td>
                    <td> {{ $order->branch->country->currency_symbol }} {{ $order->total_price_befor_tax }}</td>
                </tr>
                <tr class="total-row">
                    <td colspan="3" class="text-end">{{ trans('order.Tax') }} ({{ getSetting('tax_percentage') }}%):</td>
                    <td> {{ $order->branch->country->currency_symbol }} {{ $order->tax_value }}</td>
                </tr>
                <tr class="total-row">
                    <td colspan="3" class="text-end">{{ trans('order.Service') }}:</td>
                    <td> {{ $order->branch->country->currency_symbol }} {{ $order->service_fees }}</td>
                </tr>
                <tr class="total-row">
                    <td colspan="3" class="text-end">{{ trans('order.Total After Tax') }}:</td>
                    <td class="text-success"> {{ $order->branch->country->currency_symbol }}{{ $order->total_price_after_tax }}</td>
                </tr>
            </tbody>
        </table>
    </div>
