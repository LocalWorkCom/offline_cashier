                            ...
                            <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">@lang('branch_menu_addon.Dish')</th>
                                ...
                                <tbody>
                                    @foreach ($branch_menu_addons as $branch_menu_addon)
                                        <tr>
                                            ...
                                            <td>{{ $branch_menu_addon->branches?->name_site ?? '' }}</td>
                                            <td>{{ $branch_menu_addon->branchMenuAddonCategories?->addonCategories?->name_site ?? '' }}
                                            </td>
                                            <td>{{ $branch_menu_addon->dishes?->name_site ?? '' }}</td>
                                            <td>{{ $branch_menu_addon->dishAddons?->addons?->name_site ?? '' }}</td>
                                            <td id="branch_menu_addon_status_{{ $branch_menu_addon->id }}">
                                                @if ($branch_menu_addon->is_active == 1)
                                                    <span
                                                        class="badge bg-success">{{ __('branch_menu_addon.Active') }}</span>
                                                @else
                                                    <span
                                                        class="badge bg-secondary">{{ __('branch_menu_addon.NotActive') }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if (auth('admin')->user()->hasPermissionTo('view branch_menu_addons', 'admin'))
                                                    ...
                                                    <a href="javascript:void(0);"
                                                        ...
                                                        data-id="{{ $branch_menu_addon->id }}" data-bs-toggle="modal"
                                                        data-bs-target="#showModal">
                                                        @lang('category.show') <i class="ri-eye-line"></i>
                                                    </a>
                                                @endif
                                                @if (auth('admin')->user()->hasPermissionTo('update branch_menu_addons', 'admin'))
                                                    <!-- Edit Button -->
                                                    <button type="button"
                                                ...
                                                @if (auth('admin')->user()->hasPermissionTo('active branch_menu_addons', 'admin'))
                                                    <button type="button"
                                                        id="branch_menu_addon_activation_{{ $branch_menu_addon->id }}"
                                                        onclick="change_status_item({{ $branch_menu_addon->id }})"
                                                        class="btn btn-{{ $branch_menu_addon->is_active == 1 ? 'danger' : 'success' }}-light btn-wave">
...
<script>
    $(document).ready(function() {
        ...
        $('.edit-menu-btn').on('click', function () {
            ...
            const getUrl = "{{ route('branch.menu.addons.show', 'id') }}".replace('id', dishMenuId);
            const editUrl = "{{ route('branch.menu.addons.update', 'id') }}".replace('id', dishMenuId);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    const addons = Array.isArray(data.menus_integration_dish_addons)
                        ? data.menus_integration_dish_addons
                        : [];
                    ...
                    if (addons.length > 0) {
                        ...
                        integrationIndex = addons.length;
                    } else {
                        integrationsHtml = `
        ...
        function buildIntegrationItem(item, dishMenuId, branchMenuId, index) {
            ...
            return `
                <li class="list-group-item integration-item" data-index="${index}">
                    ...
                    <input type="hidden" name="menus_integration_dish_addons[${index}][menus_integration_id]" value="${integrationId}">
                    <input type="hidden" name="menus_integration_dish_addons[${index}][id]" value="${item.id ?? ''}">
                    <input type="hidden" name="menus_integration_dish_addons[${index}][branch_menu_id]" value="${branchMenuId}">
                    <input type="hidden" name="menus_integration_dish_addons[${index}][menus_addon_id]" value="${dishMenuId}">

                    <div class="mb-2">
                        <label class="form-label d-block mb-1">Price</label>
                        <input type="number" step="0.01" class="form-control"
                            name="menus_integration_dish_addons[${index}][price]"
        ...
        $('.show-menu-addon-btn').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#show-menu-addon-category').text(data.branch_menu_addon_categories
                        .addon_categories.name_site);
                    ...
                    if (data.is_active == 1) {
                        $('#show-menu-addon-activation').text(
                            '{{ __('branch_menu_addon.Active') }}');
                    } else {
                        $('#show-menu-addon-activation').text(
                            '{{ __('branch_menu_addon.NotActive') }}');
                    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.is_active == 1) {
                            $('#branch_menu_addon_status_' + dishMenuId).text(
                                '{{ __('branch_menu_addon.Active') }}');
                            $('#branch_menu_addon_activation_' + dishMenuId).text(
                                '{{ __('branch_menu_addon.NotActive') }}');
                        } else {
                            $('#branch_menu_addon_status_' + dishMenuId).text(
                                '{{ __('branch_menu_addon.NotActive') }}');
