                                        ...
                                        <form id="edit-menu-form" action="" method="POST" class="needs-validation"
                                            ...
                                            <div class="modal-body">
                                                <div class="row gy-4">
                                                    ...
                                                    <div class="col-xl-12 col-lg-8 col-md-8 col-sm-12">
                                                        <label for="branch" class="form-label">@lang('branch_menu_addon.MenuIntegrations')</label>
        ...
        $('.edit-menu-btn').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.menus_integration_dishes && data.menus_integration_dishes.length > 0) {
                        data.menus_integration_dishes.forEach(function(dish_menu, index) {
                            if (dish_menu.menus_integrations) {
                                integrationsHtml += `
                                    <li class="list-group-item">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <span class="fw-bold">${dish_menu.menus_integrations.name ?? '—'}</span>
                                            <input 
                                                type="number" 
                                                step="0.01"
                                                class="form-control w-50"
                                                name="menus_integration_dishes[${index}][price]"
                                                value="${dish_menu.price ?? ''}"
                                                placeholder="Price">
                                        </div>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" 
                                                    name="menus_integration_dishes[${index}][is_taxed]" 
                                                    value="1" ${dish_menu.is_taxed == 1 ? 'checked' : ''}>
                                                <label class="form-check-label">Yes</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" 
                                                    name="menus_integration_dishes[${index}][is_taxed]" 
                                                    value="0" ${dish_menu.is_taxed == 0 ? 'checked' : ''}>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input percentage-radio" type="radio" 
                                                    data-index="${index}"
                                                    name="menus_integration_dishes[${index}][is_percentage]" 
                                                    value="1" ${dish_menu.is_percentage == 1 ? 'checked' : ''}>
                                                <label class="form-check-label">Yes</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input percentage-radio" type="radio" 
                                                    data-index="${index}"
                                                    name="menus_integration_dishes[${index}][is_percentage]" 
                                                    value="0" ${dish_menu.is_percentage == 0 ? 'checked' : ''}>
                                                <label class="form-check-label">No</label>
                                            </div>
                                        </div>

                                        <div class="mt-2 percentage-amount-field" id="percentage-amount-${index}" style="display: ${dish_menu.is_percentage == 1 ? 'block' : 'none'};">
                                            <label class="form-label d-block mb-1">Percentage Amount:</label>
                                            <input 
                                                type="text"
                                                class="form-control"
                                                name="menus_integration_dishes[${index}][percentage_amount]"
                                                value="${dish_menu.percentage_amount ?? ''}"
                                                placeholder="Enter % amount">
                                        </div>

                                        <input type="hidden" name="menus_integration_dishes[${index}][id]" value="${dish_menu.id}">
                                    </li>`;
                            }
                        });
                    } else {
                    ...
                    // Toggle percentage field on radio change
                    $(document).on('change', '.percentage-radio', function() {
                        const index = $(this).data('index');
                        const value = $(this).val();
                        if (value == '1') {
                            $(`#percentage-amount-${index}`).show();
                        } else {
                            $(`#percentage-amount-${index}`).hide();
                        }
                    });
