                    ...
                    <div class="card custom-card">
                        <div class="card-header" style="display: flex; justify-content: space-between;">
                            <div class="card-title">
                                @lang('contactInformation.contact_information_setting')
                            </div>

                            <div>
                                @if (auth('admin')->user()->hasPermissionTo('create company_profile_setting', 'admin'))
                                    <a href="{{ route('contact_information_setting.create') }}"
                        ...
                        <div class="card-body">
                            ...
                            <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        <th scope="col">@lang('contactInformation.ID')</th>
                                        <th scope="col">@lang('contactInformation.CompanyAddress')</th>
                                        <th scope="col">@lang('contactInformation.PhoneNumber')</th>
                                        <th scope="col">@lang('contactInformation.Email')</th>
                                        <th scope="col">@lang('contactInformation.WebsiteLink')</th>
                                        <th scope="col">@lang('contactInformation.company_id')</th>
                                        <th scope="col">@lang('contactInformation.Actions')</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($contactInformationSettings as $setting)
                                        <tr>
                                            <td>{{ $setting->id }}</td>
                                            <td>{{ $setting->company_address }}</td>
                                            <td>{{ $setting->phone_number }}</td>
                                            <td>{{ $setting->email }}</td>
                                            <td><a href="{{ $setting->website_link }}"
                                                    target="_blank">{{ $setting->website_link }}</a></td>
                                            <td>{{ app()->getLocale() == 'ar'
                                            ...
                                            <td>
                                                @if (auth('admin')->user()->hasPermissionTo('view company_profile_setting', 'admin'))
                                                    <a href="{{ route('contact_information_setting.show', $setting->id) }}"
                                                        class="btn btn-info-light btn-wave">
                                                        @lang('contactInformation.show') <i class="ri-eye-line"></i>
                                                    </a>
                                                @endif

                                                @if (auth('admin')->user()->hasPermissionTo('update company_profile_setting', 'admin'))
                                                    <a href="{{ route('contact_information_setting.edit', $setting->id) }}"
                                                        class="btn btn-orange-light btn-wave">
                                                        @lang('contactInformation.edit') <i class="ri-edit-line"></i>
                                                    </a>
                                                @endif

                                                @if (auth('admin')->user()->hasPermissionTo('delete company_profile_setting', 'admin'))
                                                    <form class="d-inline" id="delete-form-{{ $setting->id }}"
                                                        action="{{ route('contact_information_setting.delete', $setting->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" onclick="delete_item('{{ $setting->id }}')"
                                                            class="btn btn-danger-light btn-wave">
                                                            @lang('contactInformation.delete') <i class="ri-delete-bin-line"></i>
                                                        </button>
                                                    </form>
                                                @endif
                                            </td>
