                                            ...
                                            <td>
                                                @if (auth('admin')->user()->hasPermissionTo('view cashier_machines', 'admin'))
                                                    <a href="javascript:void(0);"
                                                        ...
                                                        data-id="{{ $mashien->id }}" data-name-ar="{{ $mashien->name_ar }}"
                                                        data-name-en="{{ $mashien->name_en }}"
                                                        data-device_id="{{ $mashien->device_id }}"
                                                        data-branch_id="{{ $mashien->branches->getNameAttribute2() }}"
                                                       data-employees="{{ json_encode($mashien->employees ? $mashien->employees->pluck('id')->toArray() : []) }}"                                                       data-date="{{ $mashien->date }}" data-bs-toggle="modal"
...
{{--                            </div>--}}
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-group" id="add-employee-container">
                                    <p class="text-muted">@lang('mashiens.select_branch_first')</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Cache all employees data from server
            const allEmployees = @json($employees);

            // Function to filter employees by branch ID
            function filterEmployeesByBranch(branchId) {
                if (!branchId) return [];
                return allEmployees.filter(employee => employee.branch_id == branchId);
            }

            // Function to render employee checkboxes
            function renderEmployeeCheckboxes(employees, containerId, isEditModal = false, assignedEmployees = []) {
                const container = document.getElementById(containerId);
                container.innerHTML = '';

                if (employees.length === 0) {
                    container.innerHTML = `<p class="text-muted">@lang('mashiens.no_employees_in_branch')</p>`;
                    return;
                }

                employees.forEach(employee => {
                    const checkboxId = isEditModal ?
                        `edit-employee-${employee.id}` :
                        `employee-${employee.id}`;

                    const isChecked = assignedEmployees.includes(employee.id);
            ...
            // Initialize modals and form validation
            ...
            const deviceIdInput = document.getElementById('edit-device_id');
            const branchSelect = document.getElementById('edit-branch');
            const dateInput = document.getElementById('edit-date');

            // Add modal branch change handler
            document.getElementById('branch').addEventListener('change', function() {
                const branchId = this.value;
                const employees = filterEmployeesByBranch(branchId);
                renderEmployeeCheckboxes(employees, 'add-employee-container');
            });

            // Edit modal branch change handler
            branchSelect.addEventListener('change', function() {
                const branchId = this.value;
                const employees = filterEmployeesByBranch(branchId);
                renderEmployeeCheckboxes(employees, 'employee-checkboxes', true);
            });
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const deviceId = this.getAttribute('data-device_id');
                    const branchId = this.getAttribute('data-branch_id');
                    const date = this.getAttribute('data-date');
                    const assignedEmployees = JSON.parse(this.getAttribute('data-employees') || '[]');
                    ...
                    if (branchSelect) {
                        Array.from(branchSelect.options).forEach(option => {
                            option.selected = option.value === branchId;
                        });

                        // Load employees for this branch
                        const employees = filterEmployeesByBranch(branchId);
                        renderEmployeeCheckboxes(employees, 'employee-checkboxes', true, assignedEmployees);
                    }
                });
            });
            ...
            showButtons.forEach(button => {
                button.addEventListener('click', function() {
                    document.getElementById('show-name-ar').textContent = this.getAttribute('data-name-ar') || 'N/A';
                    document.getElementById('show-name-en').textContent = this.getAttribute('data-name-en') || 'N/A';
                    document.getElementById('show-device_id').textContent = this.getAttribute('data-device_id') || 'N/A';
                    document.getElementById('show-branch_id').textContent = this.getAttribute('data-branch_id') || 'N/A';
                    document.getElementById('show-date').textContent = this.getAttribute('data-date') || 'N/A';
