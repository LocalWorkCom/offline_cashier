            ...
            <nav>
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a href=""
                            onclick="window.location.href='{{ route('faqs.list') }}'">@lang('term.FAQs')</a></li>
                </ol>
            </nav>
        </div>
    </div>
                ...
                <div class="col-xl-12">
                    <div class="card custom-card">
                        ...
                        justify-content: space-between;">
                            ...
                            @if (auth('admin')->user()->hasPermissionTo('create faqs', 'admin'))
                                    ...
                                    @foreach ($faqs as $faq)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if (auth('admin')->user()->hasPermissionTo('update faqs', 'admin'))
                                                    <a href="{{ route('faq.edit', $faq->id) }}"
                                                        class="btn btn-orange-light btn-wave">@lang('category.edit') <i
                                                            class="ri-edit-line"></i></a>
                                                @endif
                                                @if (auth('admin')->user()->hasPermissionTo('delete faqs', 'admin'))
                                                    <form class="d-inline" id="delete-form-{{ $faq->id }}"
                                                        action="{{ route('faq.delete', $faq->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" onclick="delete_item('{{ $faq->id }}')"
