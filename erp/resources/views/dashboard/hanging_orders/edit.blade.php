...
@section('content')
    <div class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <h4 class="fw-medium mb-0">@lang('complaints.EditHangingOrdersDelivery')</h4>
        <div class="ms-sm-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('hanging-orders.list') }}">@lang('complaints.HangingOrdersDeliveries')</a></li>
                    <li class="breadcrumb-item active" aria-current="page">@lang('complaints.EditHangingOrdersDelivery')</li>
                </ol>
            </nav>
        </div>
    </div>
                ...
                <div class="col-xl-12">
                    <div class="card custom-card">
                        <div class="card-header">
                            <div class="card-title">@lang('complaints.EditHangingOrdersDelivery')</div>
                        </div>
                        <div class="card-body">
                            @if ($errors->any())
                                ...
                                @endforeach
                            @endif
                            <form method="POST" action="{{ route('hanging-order.update', $complaint->id) }}" enctype="multipart/form-data" class="needs-validation" novalidate>
                                @csrf
                                @method('PUT')
                                <div class="row gy-4">
                                    <div class="col-xl-12">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input type="radio" name="status" value="hold"
                                                   class="form-check-input" {{ $complaint->status == 'hold'? 'checked' : '' }}
                                                   required>
                                            <label class="form-check-label">@lang('complaints.Pending')</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input type="radio" name="status" value="done"
                                                   class="form-check-input" {{ $complaint->status == 'done' ? 'checked' : '' }}
                                                   required>
                                    ...
                                    <div class="col-xl-6 col-lg-8 col-md-8 col-sm-12">
                                        ...
                                        <p class="form-text">{{ $complaint->order->Client->name ?? '' }}</p>
                                    </div>

                                    <div class="col-xl-6 col-lg-8 col-md-8 col-sm-12">
                                        <label class="form-label">@lang('complaints.ClientPhone')</label>
                                        <p class="form-text">{{ $complaint->order->Client->phone ?? '' }}</p>
                                    </div>

                                    <div class="col-xl-6 col-lg-8 col-md-8 col-sm-12">
                                        <label class="form-label">@lang('complaints.Delivery')</label>
                                        <p class="form-text">{{ $complaint->employee->first_name. ' ' .$complaint->employee->last_name ?? '' }}</p>
                                    </div>
