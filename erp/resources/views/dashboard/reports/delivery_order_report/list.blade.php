...
@section('content')
    ...
    <div class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <h4 class="fw-medium mb-0">@lang('report.delivery_order_report')</h4>
        <div class="ms-sm-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.home') }}">@lang('sidebar.Main')</a></li>
                    <li class="breadcrumb-item active" aria-current="page"><a href=""
                            onclick="window.location.href='{{ route('reports.delivery_order_report.list') }}'">@lang('report.delivery_order_report')</a>
                    </li>
                </ol>
            </nav>
        </div>
                ...
                <div class="col-xl-12">
                    <div class="card custom-card">
                        ...
                        justify-content: space-between;">
                            <div class="card-title">@lang('report.delivery_order_report')</div>
                        ...
                        <div class="card-body">
                            ...
                            <form method="GET" action="{{ route('reports.delivery_order_report.list') }}">
                                <div class="row">
                                    <!-- Existing filters -->
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" name="phone_number" class="form-control"
                                            value="{{ request('phone_number') }}" placeholder="@lang('report.SearchByPhone')">
                                    </div>

                                    <div class="col-md-3">
                                        <label>@lang('report.OrderNumber')</label>
                                        <input type="text" name="order_number" class="form-control"
                                            value="{{ request('order_number') }}" placeholder="@lang('report.SearchByOrderNumber')">
                                    </div>
                                ...
                                <div class="col-md-3">
                                    <div class="stats-box">
                                        <span class="badge bg-secondary">
                                            @lang('report.TotalCancelledOrders'): {{ $totals['TotalCancelledOrders'] }}
                                        </span>
                                        <span class="badge bg-success">
                                            @lang('report.TotalComplineOrders'): {{ $totals['TotalComplineOrders'] }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>@lang('report.CancellationAndComplainDetails')</th>
                                ...
                                <tbody>
                                    @forelse($DeliveryReport as $delivery)
                                        <tr>
                                            {{-- Delivery Info --}}
                                            <td>{{ is_array($delivery) ? $delivery['delivery_id'] ?? 'N/A' : 'N/A' }}
                                            </td>
                                            <td>
                                                @php
                                            ...
                                            <td>{{ is_array($delivery) ? (is_array($delivery['total_orders'] ?? null) ? '0' : $delivery['total_orders'] ?? '0') : '0' }}
                                            </td>
                                            {{-- Cancellations --}}
                                            <td>
                                                @foreach ($delivery['cancellations'] as $cancellation)
                                                    <div>
                                                        ...
                                                        <strong>@lang('report.complain'):</strong>
                                                        {{ $cancellation['massage'] ?? __('report.NotAvailable') }}<br>
                                                        <strong>@lang('report.CancelReason'):</strong>
                                                        {{ $cancellation['reason_text'] }}<br> {{-- Localized reason from order_cancellation_reasons --}}
                                                        <strong>@lang('report.CancelMassage'):</strong>
                                                        {{ $cancellation['reason'] }} {{-- Custom message from cancellation_reasons.reason --}}
                                                        <br><br>
                                                    </div>
                                                    <hr>
                                                @endforeach
                                            ...
                                            @if (auth('admin')->user()->hasPermissionTo('detail waiter_table_service_report', 'admin'))
                                                <td>
                                                    <a href="{{ route('reports.delivery_order_report.show', $delivery['delivery_id']) }}?{{ http_build_query(request()->query()) }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4">@lang('report.No data available')</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
