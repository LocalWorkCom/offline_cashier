...
@section('content')
    ...
    <div class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <h4 class="fw-medium mb-0">@lang('order.tables_cancelled_orders')</h4>
        <div class="ms-sm-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="javascript:void(0);">@lang('sidebar.Main')</a></li>
                    <li class="breadcrumb-item active" aria-current="page">@lang('order.tables_cancelled_orders')</li>
                </ol>
            </nav>
        </div>
    </div>
                            ...
                            <table id="orders-table" class="table table-bordered text-nowrap" style="width:100%">
                                ...
                                <tr>
                                    ...
                                    @if ($hasCancelledOrders)
                                        <th>@lang('order.reason_for_cancellation')</th>
                                        <th>@lang('order.type2')</th>
                                ...
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        @endif
                                        <td>
                                            @if (auth('admin')->user()->hasPermissionTo('detail report_booking_cancellation', 'admin'))
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#filter-tabs .nav-link').on('click', function() {
                ...
                if (status === 'all') {
                    table.columns(9).search('').draw(); // Status column is index 9
                } else {
            ...
            $('#order-number-filter').on('keyup', function() {
                table.columns(2).search(this.value).draw(); // Order number column is index 2
            });

            // Invoice number filter
            $('#invoice-number-filter').on('keyup', function() {
                table.columns(3).search(this.value).draw(); // Invoice number column is index 3
            });

            // Table filter
            $('#table-filter').on('keyup', function() {
                table.columns(1).search(this.value).draw(); // Table column is index 1
            });
            ...
            $('#branch-filter').on('change', function() {
                ...
                if (branchId === '') {
                    table.columns(6).search('').draw(); // Branch column is index 6
                } else {
            ...
            $('#date-from, #date-to').on('change', function() {
                ...
                $.fn.dataTable.ext.search.push(
                    function(settings, data, dataIndex) {
                        const orderDate = new Date(data[4]).getTime(); // Date column is index 4
