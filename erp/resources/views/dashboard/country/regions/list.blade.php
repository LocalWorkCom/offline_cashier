...
@section('content')
    ...
    <div class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <h4 class="fw-medium mb-0">@lang('country.regions')</h4>
        <div class="ms-sm-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">@lang('country.regions')</li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <div class="card custom-card">
                        ...
                        justify-content: space-between;">
                            <div class="card-title">
                                @lang('country.regions')</div>
                            @if (auth('admin')->user()->hasPermissionTo('create regions', 'admin'))
                                <button type="button" class="btn btn-primary label-btn" data-bs-toggle="modal"
                                    data-bs-target="#exampleModal">
                                    <i class="fe fe-plus label-btn-icon me-2"></i>
                                    @lang('country.Addregions')
                                </button>
                            @endif
                            <!-- Add country Modal -->
                            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('region.store') }}" method="POST" class="needs-validation"
                                            novalidate>
                                            @csrf
                                            @if ($errors->any())
                                            ...
                                            <div class="modal-header">
                                                <h6 class="modal-title" id="exampleModalLabel1">@lang('country.Addregions')</h6>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row gy-4">
                                                    <input type="text" class="form-control"  name="city_id" value="{{ $city }}"
                                                        hidden>
                                                    <!-- Arabic Name Input -->
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form id="edit-country-form" action="" method="POST"
                                            ...
                                            @endif
                                            <div class="modal-header">
                                                <h6 class="modal-title" id="editModalLabel">@lang('country.Editregions')</h6>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row gy-4">
                                                    <input type="text" class="form-control" id="edit-region"  name="city_id" value="{{ $city }}"
                                                    hidden>
                                                    <!-- Arabic Name Input -->
                                                    <div class="col-xl-6 col-lg-8 col-md-8 col-sm-12">
                            ...
                            <table id="file-export" class="table table-bordered text-nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">@lang('country.cityname')</th>
                                    ...
                                    @foreach ($regions as $region)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if (auth('admin')->user()->hasPermissionTo('update regions', 'admin'))
                                                    <!-- Edit Button -->
                                                    <button type="button"
                                                        class="btn btn-orange-light btn-wave edit-country-btn"
                                                        data-id="{{ $region->id }}" data-name-ar="{{ $region->name_ar }}"
                                                        data-name-en="{{ $region->name_en }}"
                                                        data-region="{{ $region->city_id }}"
                                                        data-route="{{ route('region.update', ':id') }}"
                                                        data-bs-toggle="modal" data-bs-target="#editModal">
                                                        @lang('category.edit') <i class="ri-edit-line"></i>
                                                    </button>
                                                @endif
                                                @if (auth('admin')->user()->hasPermissionTo('delete regions', 'admin'))
                                                    <!-- Delete Button -->
                                                    <button type="button" onclick="delete_item('{{ $region->id }}')"
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const cityInput = document.getElementById('edit-region');
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const city = this.getAttribute('data-region');
        ...
        function delete_item(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('region.delete', ':id') }}'.replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'DELETE'
