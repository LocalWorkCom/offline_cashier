...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('currency_exchange_logs', function (Blueprint $table) {
            ...
            $table->json('log_values')->nullable()->comment('all relation must be added with name not id, you must send key this mean column name and value this mean the data value');
            $table->timestamp('log_timestamp')->nullable()->comment('exact time of action');
            $table->unsignedBigInteger('created_by')->nullable();
            $table->softDeletes();
            $table->timestamps();

            $table->foreign('currency_exchange_id')->references('id')->on('currency_exchanges')->onUpdate('cascade');
