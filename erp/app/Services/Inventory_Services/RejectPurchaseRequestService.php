...
class RejectPurchaseRequestService
{
    ...
    public function index(Request $request)
    {
        ...
        $reasons = RejectPurchaseRequest::query();
        ...
        $formatted = ReasonRejectResource::collection($result['data'])->resolve();
    ...
    public function show(Request $request, $id)
    {
        ...
        $reasons = RejectPurchaseRequest::find($id);
        if (!$reasons) {
...
app()->setLocale($lang);
        $validator = Validator::make($request->all(), [
            'name_ar' => 'required|string|min:3|max:100|regex:/^[\p{Arabic}A-Za-z0-9\s\-\(\)]+$/u',
            'name_en' => 'required|string|min:3|max:100|regex:/^[\p{Arabic}A-Za-z0-9\s\-\(\)]+$/u',
...
//        }
        ...
        $educationLevel = new RejectPurchaseRequest();
...
//        }
//
//        $exists_ar = RejectPurchaseRequest::where('id', '!=', $id)->where('name_ar', $request->name_ar)->exists();
//        $exists_en = RejectPurchaseRequest::where('id', '!=', $id)->where('name_en', $request->name_en)->exists();
//
...
//        }
    ...
    public function delete(Request $request, $id)
    {
        ...
        if ($reasons->purchaseRequests()->exists()) {
            $reasons->is_active = 0;
            $reasons->updated_by = authActionSave()['by'];
            $reasons->updated_by_type = authActionSave()['type'];
            $reasons->save();
