...
class OrderServiceOld
{
    ...
    function validateOrderItem($items, $branchId, $type)
    {
        foreach ($items as $itemIndex => $item) {
            ...
            if (!$branchDish) {
                return respondError('error', 400, ['error' => __('order.dish_not_found')]);
            }

            // Check if the dish category is active
            if ($branchDish->dish->dishCategory->is_active != 1) {

                return respondError('error', 400, ['error' => __('order.dish_category_not_active')]);
            }

            // Check if the dish category branch is active
            if ($branchDish->branchMenuCategories->is_active != 1) {
                return respondError('error', 400, ['error' =>  __('order.dish_category_branch_not_active')]);
            }
            // Check if the dish is active
            if ($branchDish->is_active != 1) {
                return respondError('error', 400,  ['error' => __('order.dish_not_active')]);
            }
            $has_size = $branchDish->dish->has_sizes;
            if ($has_size && (empty($item['sizeId']))) {
                return respondError('error', 400, ['error' => __('order.dish_size_required')]);
            } elseif (!$has_size && (!empty($item['sizeId']))) {
                return respondError('error', 400, ['error' => __('order.dish_not_have_size')]);
            } elseif ($has_size && $item['sizeId']) {
                $size_id = $item['sizeId'];
                $branch_dish_size = BranchMenuSize::where('id', $size_id)->where('branch_id', $branchId)->where('is_active', 1)->first();

                if (!$branch_dish_size || $branch_dish_size->is_active != 1) {
                    return respondError('error', 400, ['error' =>  __('order.dish_size_not_active_or_deleted')]);
                }
                if ($branchDish->dish->id != $branch_dish_size->dish_id) {
                    return respondError('error', 400, ['error' =>  __('order.size_not_related_dish')]);
                }
            }
            ...
            if ($has_addon && (empty($addonCategories))) {
                ...
                if ($minAddon > 0) {
                    return respondError('error', 400, ['error' =>  __('order.addon_categories_required')]);
                }
                ...
                foreach ($addonCategories as $catIndex => $category) {
                    ...
                    if (!$BranchMenuAddonCategory || $BranchMenuAddonCategory->is_active != 1) {
                        return respondError('error', 400, ['error' =>  __('order.addon_category_not_active')]);
                    }
                    ...
                    if (!empty($addonList)) {
                        foreach ($addonList as $addon) {
                            ...
                            if (!$BranchMenuAddon || $BranchMenuAddon->is_active != 1) {
                                return respondError('error', 400,  ['error' => __('order.addon_not_active')]);
                            }
                            if ($branchDish->dish->id != $BranchMenuAddon->dish_id) {
                                return respondError('error', 400,  ['error' => __('order.addon_not_related_dish')]);
                            }
                        }
                    }
                    ...
                    if ($has_addon && ($addonCount < $minAddon || $addonCount > $maxAddon)) {
                        return respondError('error', 400,  ['error' => __("order.addon_must_be_min_max") .  $minAddon . "-" . $maxAddon]);
                    }
                }
            }
        }
    ...
    public  function validate(array $data, string $lang = 'en', $checkToken, $dir, $function_name = 'store')
    {
        ...
        $rules = [
            ...
            'type' => 'required|string|in:Takeaway,Online,dine-in,Delivery,CallCenter,reservation-table',
            'branch_id' => 'required|exists:branches,id',
            'address_id' => 'nullable|exists:client_addresses,id',
            'coupon_code' => 'nullable|exists:coupons,code',
            'make_type' => 'required|string',
            'payment_method' => 'nullable|string|in:cash,credit,online,credit_with_delivery,deposit,no_payment_required,deposit_required,full_payment_required',
            'payment_method2' => 'nullable|string|in:cash,credit,online,credit_with_delivery,deposit,no_payment_required,deposit_required,full_payment_required',
        ...
        if (!empty($data['coupon_code'])) {
            ...
            if ($coupon->apply_type == 'dish') {
                if (!checkCouponApplyDish($coupon->id, $dish_ids, $data['branch_id'])) {
                    return respondError("errors", 400, ['coupon_code' => [$lang === 'ar' ? 'هذا الكوبون غير صالح' : 'Invalid coupon']]);
                }
            }
        }
        ...
        if (!in_array($data['type'], ['Takeaway', 'reservation-table', 'dine-in'])) {
    ...
    public function store_v2(array $request, $checkToken, $type)
    {
        ...
        if (($request['type'] == 'dine-in' || $request['type'] == 'Takeaway') &&  $makeType == 'cashier') {
            ...
            if (!$cash_amount && !$credit_amount) {
                if (($cash_amount + $credit_amount) == $order->total_price_after_tax) {
                    ...
                    $this->storePaymentTransaction($order->id, $request['type'], 'cash', $created_by, $coupon?->id,  $credit_amount, $orderDeposit, $makeType, 'paid', 0, null, $totals['order_invoice_id']);
                } else {
                ...
                if ($cash_amount + $credit_amount < round($order->total_price_after_tax, 2)) {
                    DB::rollBack();
    ...
    {
        $order = Order::with('orderDetails', 'orderDetailsWithoutCancel.dish', 'Table', 'orderDetails.dish')->find($order_id);
        ...
        log::info('Order data for notification:', $order_data);
        // Notify cashier
        if ($OrderType == 'Delivery' && $UserType == 'client') {

            $cashiers = getEmployeesForNotify($branch_id,  now(), 'cashier');
            if (!$cashiers) {
                DB::rollBack();
                return respondError('Validation Error.', 404, ['error' => __('recipes.nocashiersworknowinbranch')]);
            }
            ...
            $notifyData = [
                ...
                'type' => $OrderType,
                'title_en' => "New online order",
                'description_ar' => "تم استلام طلب جديد عبر الإنترنت. الطلب [{$order->order_number}] بحاجة إلى معالجة.",
                'description_en' => "New online order received. Order [{$order->order_number}] requires processing",
            ];
            foreach ($cashiers as $cashier) {
                ...
                broadcast(new NewOrder(
                    $cashier->id,
                    $cashier->branch_id,
                    $order_data
                ));
            }
        } else if ($UserType == 'customer_service' && auth('employee')->user()->branch_id != $branch_id) {

            $callcenter = getEmployeesForNotify($branch_id,  now(), 'customer_service');

            if (!$callcenter) {
                return respondError('Validation Error.', 400, ['error' => __('recipes.nocall_centersworknowinbranch')]);
            }
            ...
            $notifyData = [
                ...
                'order_type' => $order->type,
                'order_status' => $order->status,
                'title_ar' => "تم أضافه طلب جديد ",
                'title_en' => "A New order added",
                'type' => $OrderType,
                'description_ar' => "تم استلام طلب جديد عبر الكاشير. الطلب [{$order->order_number}] بحاجة إلى معالجة.",
                'description_en' => "New cashier order received. Order [{$order->order_number}] requires processing",
            ];
            foreach ($callcenter as $callcenter) {
                addNotification(
                    'order',
                    $UserType,
                    'تم أضافه طلب جديد ' . $order->order_number,
                    'A New order added' . $order->order_number,
                    'طلب جديد',
                    'New order',
                    $callcenter->id,
                    $created_by,
            ...
            if ($order->type == 'dine-in') {
                $waiters = getEmployeesForNotify($branch_id,  now(), 'waiter');

                if (!$waiters) {
                    return respondError('Validation Error.', 400, ['error' => __('recipes.nowaitersworknowinbranch')]);
                }
            ...
            $notifyData = [
                ...
                'title_ar' => "تم أضافه طلب جديد ",
                'title_en' => "A New order added",
                'description_ar' => "تم استلام طلب جديد عبر الجرسون. الطلب [{$order->order_number}] بحاجة إلى معالجة.",
                'description_en' => "New waiter order received. Order [{$order->order_number}] requires processing",
            ];
            foreach ($cashier as $cashier) {
                broadcast(new NewOrder(
                    $cashier->id,
            ...
            $notifyData = [
                ...
                'type' => $OrderType,
                'title_en' => "New order",
                'description_ar' => "تم استلام طلب جديد  . الطلب [{$order->order_number}] بحاجة إلى معالجة.",
                'description_en' => "New  order received. Order [{$order->order_number}] requires processing",
            ];
            if ($cashiers) {
                foreach ($cashiers as $cashier) {
                    addNotification(
    ...
    public function storeOrderItems(
        ...
        if ($order_edit) {
            ...
            if (!$hasPaidTransaction) {
                ...
                $responseInv = $invoiceService->editInvoice($order->id, $lang);
        ...
        foreach ($itemData as $index => $data) {
            ...
            if ($order->coupon_id && $couponApplication == 0) {
                ...
                if ($coupon->type == 'fixed') {
                    $couponValue = $coupon->value / (count($itemData) + count($addons));
        ...
        $order->update([
            'total_price_befor_tax' => $subTotal,
        ...
        return compact('subTotal', 'totalTax', 'totalService', 'total', 'order_invoice_id');
    }
    public function CalculateOrder($order_id)
    {
        ...
        foreach ($itemData as $data) {
            ...
            foreach ($addons as $addon) {
                ...
                $addon->price_before_tax = $addonPrice;
        ...
        $total = $allSubTotalAfterCoupon + $totalService + $totalTax + $Order->delivery_fees;
        $Order->total_price_befor_tax = $subTotal;
    ...
    public function CalculateItem($item_id, $quantity)
    {
        ...
        if ($coupon_id) {
            ...
            if ($coupon->type == 'fixed') {
                $couponValue = $coupon->value;
                $subTotalAfterCoupon = $total - $couponValue;
            } else {
                $couponValue = calcCoupon($total, $coupon);
                $subTotalAfterCoupon = applyCoupon($total, $coupon);
            }
        } else {
            $subTotalAfterCoupon = $total;
        }
        if ($Order->type  == 'dine-in' || $Order->type == "reservation-table") {
            $price_before_tax = $tax_application == 1 ? applyTax($subTotalAfterCoupon * $quantity, $tax_percentage, $tax_application) : $subTotalAfterCoupon * $quantity;
            ...
            $service_value =  0;
            $price_before_tax = $tax_application == 1 ? applyTax($subTotalAfterCoupon * $quantity, $tax_percentage, $tax_application) : $subTotalAfterCoupon * $quantity;
            $tax_value = $tax_application == 1 ? CalculateTax($tax_percentage, $subTotalAfterCoupon * $quantity) : $price_before_tax * ($tax_percentage / 100);
            $price_after_tax = $price_before_tax + $tax_value;
        }
        ...
        if (!empty($OrderAddonsArray)) {
            foreach ($OrderAddonsArray as $OrderAddonsArr) {
                if ($OrderAddonsArr) {
                    ...
                    $price = $addon->price;
                    if ($Order->type == 'dine-in' || $Order->type == "reservation-table") {
                        $price_before_tax = $tax_application == 1 ? applyTax($price * $quantity, $tax_percentage, $tax_application) : $price * $quantity;
                        $service_value =  $service_fees_type != 'fixed' ? ($price_before_tax * ($service_fees_value / 100)) : 0;
                        $priceAfterService =  $price_before_tax +  $service_value;
                        $tax_value = $tax_application == 1 ? CalculateTax($tax_percentage,  $priceAfterService) : $priceAfterService * ($tax_percentage / 100);
                        $price_after_tax = $priceAfterService + $tax_value;
                        ...
                        $service_value = 0;
                        $price_before_tax = $tax_application == 1 ? applyTax($price * $quantity, $tax_percentage, $tax_application) : $price * $quantity;
                        $tax_value = $tax_application == 1 ? CalculateTax($tax_percentage, $price * $quantity) : $price_before_tax * ($tax_percentage / 100);
                        $price_after_tax = $price_before_tax + $tax_value;
                    }
                    $addons[] = [
        ...
        $response = [
            ...
            'dish_coupon_value' => $dish_coupon_value,
            'addons' => $addons,
    ...
    public function requestCancellation(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                return respondErrorData('validation_error', 422, $validator->errors());
            }
            ...
            if ($type == 'partial') {
                ...
                foreach ($order_details_ids as $orderDetailId) {
                    ...
                    if ($requestedQty == $orderDetail->quantity) {
                        ...
                        $caluclate_dish = $invoiceService->CalculateItemOrder($order_id, [$newOrderDetail->id], [], "dish", [$requestedQty]); //type = dish or addon or order
                        ...
                        if (count($addons) > 0) {
                            ...
                            foreach ($addons as $addon) {
                                ...
                                $caluclate_addon = $invoiceService->CalculateItemOrder($order_id, [], [$newAddon->id], "addon", [$requestedQty]); //type = dish or addon or order
                                $add_invoice_details = new InvoiceDetails();
                                $add_invoice_details->invoice_id = $invoiceDetail->invoice_id;
                        ...
                        $caluclate_dish = $invoiceService->CalculateItemOrder($order_id, [$orderDetail->id], [], "dish", [$remainingQty]); //type = dish or addon or order
    ...
    public function orderCancel(Request $request)
    {
        ...
        if ($request->type == 2) {
            ...
            $hasPaidTransaction = OrderTransaction::where('order_id', $request->order_id)
                ...
                ->exists();
            if ($order_details) {
                if ($request->quantity == $order_details->quantity) {
                    $old_qty = $order_details->quantity;
                ...
                } else if ($request->quantity > $order_details->quantity) {
                    return response()->json([
                        'code' => 400,
                        'status' => false,
                    ...
                    $old_qty = $order_details->quantity;
                    $new_qty = $order_details->quantity - $request->quantity;
                    $order_details->quantity = $new_qty;
                    $order_details->modify_by = $created_by;
                    $order_details->save();
                    //update addon quantity related to this dish 
                    OrderAddon::where('order_details_id', $order_details->id)->update(['quantity' => $new_qty, 'modify_by' => $created_by]);
                }
                ...
                if ($request->quantity != $order_details->quantity) {
                    $item_calculate = $this->CalculateItem($order_details->id, $order_details->quantity - $request->quantity);
                ...
                if ($hasPaidTransaction) {
                    if ($old_qty >= $new_qty) {
                        ...
                        $this->storePaymentTransaction($request->order_id, $order->type, $request->payment_method, $created_by, null, $refund_price_qty, null, $order->make_type, 'paid', 1, null);
                    }
                } else {
                ...
                $order_details_with_prices = OrderDetail::with('dishAddons')
                    ...
                    ->toArray();
                      $data = [
                    'order_id' => $order->id,
                    'dish_ids' => $order_details_with_prices,
                    'status' => null,
                    'order_status' => $order->status,
            ...
            if ($order->type == 'dine-in' && $order->table_id) {
                ...
                $responseData = $notification_response->original;
                // if (!$responseData['status']) {
            ...
            $hasPaidTransaction = OrderTransaction::where('order_id', $request->order_id)
                ...
                ->exists();
            if ($hasPaidTransaction) {
                $this->storePaymentTransaction($request->order_id, $order->type, $request->payment_method, $created_by, null, $order->total_price_after_tax, null, $order->make_type, 'paid', 1, null);
            }
            //call invoice function
            $order_details_ids = $order->orderDetails->pluck('id')->toArray();
            $order_addon_ids = $order->orderDetails->flatMap(function ($item) {
                return $item->dishAddons->pluck('id');
            })->toArray();
            $data = [
                'orderId' => $order->id,
                'status' => 'cancelled',
    ...
    public function orderEditItem(Request $request, $type)
    {
        ...
        if ($order->status != "pending"  || $order->orderTransactions->contains('payment_status', 'paid')) {
        ...
        if ($request->has('addon_categories')) {
            if ($request->addon_categories != []) {
                ...
                foreach ($request->addon_categories as $addon_category) {
                    ...
                    // Cancel removed addons
                    OrderAddon::where([
                        'order_details_id' => $checkOrderDetails->id,
                        'status' => 'pending'
                    ])->whereNotIn('dish_addon_id', $addons)->update(['status' => 'cancel']);
        ...
        $item_calculate2 = $this->CalculateOrder($request->order_id);
        $order->refresh();
        $order_transaction = OrderTransaction::where('order_id', $request->order_id)->where('is_refund', 0)->where('payment_status', 'unpaid')->first();
        $order_transaction->paid = $order['total_price_after_tax'];
        $order_transaction->save();
