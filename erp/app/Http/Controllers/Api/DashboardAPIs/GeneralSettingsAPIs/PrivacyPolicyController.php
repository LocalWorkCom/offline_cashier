...
use App\Services\SettingsServices\PrivacyPolicyService;
...
class PrivacyPolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $query = $this->PrivacyPolicyService->index($request);

            // Handle pagination or get all
            $response = paginateOrGetAll($query, $request, null);
            $response['data'] = $response['data']->map(function($policy) use($lang) {
                $policy->active = $policy->active == 1 ? ($lang == 'en' ? 'Active' : 'نشط') : ($lang == 'en' ? 'Not Active' : 'غير نشط');
                return $policy;
            });

            return ResponseWithSuccessDataPaginated($lang, $response, 1);
        } catch (\Exception $e) {
            Log::error('Error fetching PrivacyPolicy : ' . $e->getMessage());
    ...
    public function store(Request $request)
    {
        ...
        $policy = $this->PrivacyPolicyService->store($request, null);
    ...
    public function show(Request $request, $id)
    {
        ...
        $exists = PrivacyPolicy::where('id', $id)->exists();

        if (!$exists) {
            return respondError(__('branch_menu_category.not_found'), 404);
        }
        try {
            $policy = $this->PrivacyPolicyService->show($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $data = $this->PrivacyPolicyService->update($request, $id);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            $response = $this->PrivacyPolicyService->destroy($id, $lang);
            ...
            $message = $lang === 'ar' ? 'تم حذف سياسة الخصوصية بنجاح' : 'PrivacyPolicy deleted successfully';
